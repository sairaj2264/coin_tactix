...
export const fetchTechnicalIndicators = createAsyncThunk(
  ...
  async ({ symbol, indicators }, { rejectWithValue }) => {
    try {
      const response = await marketAPI.getTechnicalIndicators(symbol, indicators)
      return { symbol, indicators: response.data }
    } catch (error) {
      return rejectWithValue(error.response?.data?.message || 'Failed to fetch technical indicators')
    }
  }
)
...
export const fetchOnChainMetrics = createAsyncThunk(
  ...
  async ({ symbol }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to fetch on-chain metrics')
    }
  }
)
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchTechnicalIndicators.fulfilled, (state, action) => {
        state.technicalIndicators[action.payload.symbol] = action.payload.indicators
      })
